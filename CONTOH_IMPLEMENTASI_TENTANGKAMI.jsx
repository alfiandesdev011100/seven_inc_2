// ✅ IMPLEMENTASI COMPLETE - TentangKamiFull.jsx (API-driven)
// Gunakan ini sebagai reference untuk update file Anda
...
// ✅ KEEP FALLBACK DATA (untuk saat API down)
const CardArray = [
...
const TentangKamiFull = () => {
    ...
    // ✅ MODIFIED: isLast sekarang dynamic dari API atau fallback
    const coreValues = about?.core_values || CardArray;
    const heroImages = about?.hero_images || HERO_IMAGES;
    const isLast = startIndex + VISIBLE >= coreValues.length;

    // ✅ NEW: Fetch data dari API
    useEffect(() => {
        const fetchAbout = async () => {
            try {
                setLoading(true);
                const response = await fetch(
                    "http://127.0.0.1:8000/api/public/about"
                );
                ...
                // ✅ FALLBACK: Tetap render dengan data hardcoded
                setAbout(null);
            } finally {
                setLoading(false);
            }
    ...
    useEffect(() => {
        // ✅ MODIFIED: Use heroImages length (dynamic)
    ...
    useEffect(() => {
        ...
        const id = setInterval(() => {
            setAnimDir("next");
            setStartIndex((prev) => (prev + VISIBLE >= coreValues.length ? 0 : prev + 1));
        }, AUTO_MS);
        return () => clearInterval(id);
    }, [paused, coreValues.length]);
    ...
    if (loading) {
        return (
            <Layout>
                <div className="bg-white text-gray-800 pt-[110px] pb-[110px]">
                    <Container>
                        <div className="text-center py-20">
                            <p className="text-gray-600">Memuat data tentang kami...</p>
                        </div>
                    </Container>
                </div>
                <Footer />
    ...
    // ✅ NEW: Show error state (tapi tetap render dengan fallback)
    if (error && !about) {
            ...
            <div className="bg-white text-gray-800 pt-[110px] ">
                <Container>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between">
                        ...
                        {/* Tumpuk semua gambar, yang aktif opacity-100 */}
                        <div className="w-full flex justify-end">
                            <div className="relative max-w-[733px] h-[561px] w-full">
                                {/* ✅ MODIFIED: Use heroImages dari API */}
                                {heroImages.map((img, index) => (
                                    <img
                                        key={index}
                                        src={img.src || img.image}  // ✅ Handle both field names
                                        alt={img.alt || "Seven INC"}
                    ...
                    <div className="mt-[85px] flex flex-col md:flex-row justify-between gap-10 mb-[85px]">
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    {/* ✅ MODIFIED: Use coreValues dari API */}
                                    {coreValues.map((card) => (
                                        <div
                                            key={card.id}  // ✅ CHANGED: dari index ke card.id
